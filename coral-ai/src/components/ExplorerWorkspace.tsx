...
ORDER BY
  ...
  const [newSql, setNewSql] = useState('-- Write your local cluster database query here...\nSELECT * FROM supabase.billing_events LIMIT 10;');
  const [isRunning, setIsRunning] = useState(false);
  const [explorerFeedback, setExplorerFeedback] = useState<string | null>(null);
  const [queryResult, setQueryResult] = useState<QueryResult>({
    columns: ["timestamp", "pod_name", "namespace", "replicas", "cpu_usage", "status"],
    rows: [
      { timestamp: "2026-05-26T15:45:56.575277", pod_name: "worker-x-54abc", namespace: "default", replicas: 40, cpu_usage: 95.5, status: "CrashLoopBackOff" }
    ],
    timeMs: 25
  });
...
LIMIT 10;`;
  ...
  const databaseSchema: SchemaTable[] = [
    {
      ...
      columns: [
        ...
        { name: 'namespace', type: 'utf8' },
        { name: 'pod_name', type: 'utf8' },
    ...
    },
    {
      ...
      columns: [
        { name: 'id', type: 'int64', isKey: true },
        { name: 'deployment_id', type: 'utf8' },
        { name: 'service_name', type: 'utf8' },
        { name: 'version', type: 'utf8' },
        { name: 'deployed_by', type: 'utf8' },
        { name: 'deployed_at', type: 'utf8' }
      ]
    },
    {
    ...
    },
    {
      ...
      columns: [
        ...
        { name: 'target_node', type: 'utf8' },
        { name: 'protocol', type: 'utf8' },
        { name: 'action', type: 'utf8' },
        { name: 'event_desc', type: 'utf8' }
      ]
    }
  ];
  ...
  const handleExecuteQuery = async () => {
    ...
    try {
      ...
      setQueryResult({
        columns: ["timestamp", "pod_name", "namespace", "active_replicas", "cpu_cores", "exit_code_reason"],
        rows: [
          { timestamp: "2026-05-25 11:22:10 UTC", pod_name: "core-ledger-pod-68a8", namespace: "coral-prod", active_replicas: 10, cpu_cores: 4.0, exit_code_reason: "OOMKilled" },
          { timestamp: "2026-05-25 11:15:02 UTC", pod_name: "notification-worker-pod-eff1", namespace: "coral-prod", active_replicas: 10, cpu_cores: 1.2, exit_code_reason: "Running" }
        ],
        timeMs: 110
      });
    } finally {
                    ...
                    return (
                      <tr key={idx} className={bgColStyle}>
                        ...
                        {queryResult.columns.map((col) => {
                          ...
                          } else if (col === 'exit_code_reason' && val === 'OOMKilled') {
                            cellStyle = "text-rose-400 font-bold italic";
                          } else if (col === 'active_replicas') {
