...
export default function OverviewWorkspace({ onAnalyzeCard, searchFilter }: OverviewWorkspaceProps) {
  ...
  const [aiResponse, setAiResponse] = useState<{
    ...
    sql: `-- AI generated root-cause query for Kubernetes pod replicas count explosion\nSELECT \n    pod_name,\n    namespace,\n    SUM(cpu_cores) AS aggregate_cpu,\n    COUNT(replica_id) AS active_replicas,\n    last_event\nFROM k8s_pod_events\nWHERE timestamp >= '2026-05-25T00:00:00Z' AND exit_code_reason = 'OOMKilled'\nGROUP BY pod_name, namespace, last_event\nORDER BY aggregate_cpu DESC;`,
    explanation: "Horizontal Pod Autoscaler (HPA) triggered an emergency scale-up of deployment/core-ledger-pod from 3 to 10 replicas due to high queue backlog ingestion rate. The sudden memory heap limits breached caused an immediate CrashLoopBackOff.",
    insights: [
      "Backlog queue explosion forced HPA scaling to reach 10 maximum replicas limits within 1.2 minutes.",
      "Sub-node 'k8s-worker-node-1' experienced heap thread saturation before OOMKilled eviction took place.",
      "Recommend: Scale memory request limits to 4Gi and deploy backpressure throttling in deployment-spec."
    ],
    loading: false
  });
      ...
      if (data.success && data.pods) {
        ...
        if (data.pods.length > 0) {
          setSelectedPod((prev: any) => {
            const found = data.pods.find((p: any) => p.name === prev?.name);
            return found || data.pods[0];
          });
        }
      }
    } catch (err) {
  ...
  // Query simulation status on mount
  useEffect(() => {
  ...
  // Hook representing live pod streaming logs
  useEffect(() => {
    if (selectedPod) {
      setLogsLoading(true);
  ...
  const cards: AnomalyCard[] = [
    {
      ...
      trend: isSimulated ? 'up' : 'down',
      severity: isSimulated ? 'critical' : 'normal',
      history: isSimulated ? [15, 18, 12, 19, 32, 65, 87, 100] : [15, 18, 12, 19, 21, 23, 22, 24],
    ...
    },
    {
      ...
      percentage: isSimulated ? '98.2% CPU core' : '34.1% CPU Balanced',
      trend: isSimulated ? 'up' : 'down',
      severity: isSimulated ? 'critical' : 'normal',
      history: isSimulated ? [90, 85, 87, 72, 80, 91, 95, 98] : [40, 42, 38, 35, 34, 32, 35, 34],
    ...
    },
    {
      ...
      percentage: isSimulated ? '88% Claim bounds' : '42% Claim Balanced',
      trend: 'up',
      severity: isSimulated ? 'warning' : 'normal',
      history: isSimulated ? [30, 28, 35, 42, 45, 52, 60, 88] : [30, 28, 31, 32, 35, 38, 40, 42],
  ...
  const handleSimulate = async () => {
    setIsSyncing(true);
    try {
      const response = await fetch('/api/simulate-backlog', {
        method: 'POST'
      });
      ...
      if (data.success) {
        setIsSimulated(true);
        await fetchPodsData();
        // Automatically fetch new analysis metrics after triggering simulation
        await triggerAiInquest(activeAnalysisTopic);
      }
    } catch (err) {
    ...
    <div className="ml-64 mt-12 mb-6 p-8 flex flex-col gap-6 h-[calc(100vh-72px)] overflow-y-auto font-sans text-white">
      ...
      <div className="flex justify-between items-end border-b border-white/10 pb-6 shrink-0">
        ...
        <div className="flex items-center gap-3">
          {/* SIMULATE BUTTON (Step 3: Trigger the Simulation / Queue Explosion) */}
          <button
            onClick={handleSimulate}
            disabled={isSyncing}
            className="h-9 px-5 bg-gradient-to-r from-amber-600/30 to-red-600/30 hover:from-amber-600/50 hover:to-red-600/50 border border-amber-500/30 hover:border-amber-500/60 text-amber-300 font-mono text-[10px] uppercase tracking-widest rounded transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50 duration-150"
            title="Seed the local database list with telemetry failures (RDS replica explosion and EC2 spot surge / capacity dropping)"
          >
            <Activity size={12} className={isSimulated ? "animate-pulse" : ""} />
      ...
      {isSimulated && (
        <div className="bg-amber-950/20 border border-amber-500/30 text-amber-300 text-xs py-3 px-4 rounded flex items-center gap-2.5 font-mono">
          ...
          <span>Active backlog simulation running container replica spikes and worker-node limit constraints. Data synced.</span>
        </div>
      )}
