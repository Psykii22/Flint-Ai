...
import { Shield, SearchCode, Lock, Terminal, Activity } from 'lucide-react';
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 bg-[#111112] border-r border-white/10 flex flex-col py-6 z-20 font-sans">
      ...
      <div className="px-6 mb-10 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-white tracking-[0.2em] uppercase">
            Coral<span className="font-thin text-white/40">AI</span>
          </h1>
      ...
      <nav className="flex-1 flex flex-col gap-2 px-4">
        ...
        {/* Shield tab */}
        <button
          onClick={() => isAuthenticated && setScreen('shield')}
          disabled={!isAuthenticated}
          className={`flex items-center gap-3 px-4 py-3 border border-transparent rounded transition-all duration-150 text-left cursor-pointer w-full group ${
            !isAuthenticated ? 'opacity-30 cursor-not-allowed' : ''
          } ${
        ...
        >
          <Shield size={15} className={activeScreen === 'shield' ? 'text-[#c5a27a]' : 'text-white/40 group-hover:text-white transition-colors'} />
          <span className="text-[10px] uppercase tracking-[0.25em] font-semibold">Shield Firewall</span>
        </button>
