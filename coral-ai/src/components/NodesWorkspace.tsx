...
export default function NodesWorkspace({ searchFilter }: NodesWorkspaceProps) {
  ...
  // Core set of k8s pods/namespaces
  const podsList = [
    { name: 'core-ledger-pod-68a8', namespace: 'coral-prod', status: 'Running' },
    { name: 'auth-gateway-pod-f2b3', namespace: 'coral-prod', status: 'Running' },
    { name: 'notification-worker-pod-eff1', namespace: 'coral-prod', status: 'Running' },
    { name: 'user-preference-pod-a129', namespace: 'coral-prod', status: 'Running' },
    { name: 'ingress-nginx-controller', namespace: 'kube-system', status: 'Running' }
  ];

  const [activePods, setActivePods] = useState(podsList);

  const [logs, setLogs] = useState<LogEntry[]>([
    { timestamp: "11:20:00.002Z", node: "coral-prod", severity: "INFO", module: "auth-gateway-pod-f2b3", message: "Gateway proxy connection established dynamically" },
    { timestamp: "11:20:12.451Z", node: "coral-prod", severity: "INFO", module: "user-preference-api-a129", message: "Token configuration validated successfully" },
    { timestamp: "11:20:13.882Z", node: "coral-prod", severity: "WARN", module: "core-ledger-pod-68a8", message: "Latency alert: 450ms query processing latency" },
    { timestamp: "11:21:02.102Z", node: "coral-prod", severity: "INFO", module: "notification-worker-pod-eff1", message: "Backlog worker heartbeat check: OK" },
    { timestamp: "11:21:15.000Z", node: "kube-system", severity: "INFO", module: "ingress-nginx-controller", message: "Routing reload check completed successfully" },
    { timestamp: "11:21:40.772Z", node: "coral-prod", severity: "WARN", module: "core-ledger-pod-68a8", message: "Heap allocation boundary nearing upper quotas (512Mi)" }
  ]);
  ...
  useEffect(() => {
    fetch('/api/simulation-state')
      ...
      .then(data => {
        if (data.simulated) {
          setIsSimulated(true);
          // Set simulated pod crash statuses
          setActivePods([
            { name: 'core-ledger-pod-68a8', namespace: 'coral-prod', status: 'CrashLoopBackOff' },
            { name: 'auth-gateway-pod-f2b3', namespace: 'coral-prod', status: 'Running' },
            { name: 'notification-worker-pod-eff1', namespace: 'coral-prod', status: 'Running' },
            { name: 'user-preference-pod-a129', namespace: 'coral-prod', status: 'Running' },
            { name: 'ingress-nginx-controller', namespace: 'kube-system', status: 'Running' }
  ...
  // Rolling live log simulator
  useEffect(() => {
    if (!isPlaying) return;

    const namespaces = ['coral-prod', 'kube-system'];
    const mockPods = [
      { name: 'auth-gateway-pod-f2b3', ns: 'coral-prod' },
      { name: 'notification-worker-pod-eff1', ns: 'coral-prod' },
      { name: 'user-preference-pod-a129', ns: 'coral-prod' },
      { name: 'ingress-nginx-controller', ns: 'kube-system' }
    ];

    const k8sInfos = [
      'Metadata synced with local cluster config maps.',
      'Active namespace CPU quota bounds recalculated cleanly.',
      'Core thread workers verified idle state.',
      'Ingress controller synchronized with proxy routing tables.',
      'Kubelet garbage collection sweep executed successfully.'
    ];

    const k8sWarns = [
      'Resource quota limit limits are constrained near 85% limits.',
      'Ingress router detected slow upstream response callback.',
      'Micro-controller retries buffer size elevated: 12 entries.'
    ];

    const interval = setInterval(() => {
      const isAnomalous = isSimulated && Math.random() > 0.4;
      const isGeneralWarn = Math.random() > 0.8;
      const isGeneralCrit = Math.random() > 0.94;
      ...
      if (isAnomalous) {
        ...
        podName = 'core-ledger-pod-68a8';
        ns = 'coral-prod';
        
        if (severity === 'CRIT') {
          message = Math.random() > 0.5 
            ? 'Fatal Error: OutOfMemoryError - Container core-ledger-pod-68a8 terminated with exit code 137.'
            : 'Kubelet alert: Pod evicted on worker node-1 due to namespace memory starvation.';
        } else {
          message = Math.random() > 0.5
            ? 'Warn: Pod container restarting in CrashLoopBackOff state. (Restarts count: 12)'
            : 'Warn: Garbage collection overhead limit breached. JVM threads unresponsive.';
        }
      } else {
        const selPod = mockPods[Math.floor(Math.random() * mockPods.length)];
        podName = selPod.name;
        ns = selPod.ns;

        if (isGeneralCrit) {
          severity = 'CRIT';
          message = `Critical boundary violation: CPU throttle threshold breached on pod ${podName}.`;
        } else if (isGeneralWarn) {
          severity = 'WARN';
          message = k8sWarns[Math.floor(Math.random() * k8sWarns.length)];
        } else {
          message = k8sInfos[Math.floor(Math.random() * k8sInfos.length)];
        }
      }

      const newLog: LogEntry = {
        timestamp: timeStr,
        node: ns, // maps namespace
        severity,
        module: podName, // maps pod name
        message
      };

      setLogs((prev) => {
        const sliced = prev.length > 60 ? prev.slice(prev.length - 40) : prev;
    ...
    <div className="ml-64 mt-12 mb-6 p-8 flex flex-col gap-4 h-[calc(100vh-72px)] overflow-hidden font-sans text-white bg-[#0a0a0b]">
      ...
      {isSimulated && (
        <div className="bg-rose-950/20 border border-rose-500/30 text-rose-300 text-xs py-3 px-4 rounded flex items-center gap-2.5 font-mono shrink-0">
          ...
          <span>Namespace Event Warning: deployment/core-ledger-pod-68a8 crashed with OOMKilled eviction. Multiple CrashLoopBackOff restarts recognized.</span>
        </div>
      )}
